...
const initialEvacuees: Evacuee[] = [
	...
	},
	{
		...
		medicalNotes:
			"Hipertensión arterial y movilidad reducida (anda con bastón). Requiere asistencia para subir escaleras.",
...
const initialNotices: NoticeAlert[] = [
	{
		...
		message:
			"Se aguarda un contingente de 15 personas provenientes del Barrio San Cayetano en los próximos 45 minutos.",
		type: "urgent",
	...
	},
	{
		...
		message:
			"El equipo de salud iniciará la colocación de refuerzos antitetánicos en Zona A y C a partir de las 14:00 hs.",
		type: "info",
...
export const ShelterProvider: React.FC<{children: React.ReactNode; initialUser: UserProfile}> = ({
	...
	const addEvacuee = (evacueeData: Omit<Evacuee, "id" | "entryTimestamp">) => {
		...
		setToastMessage(
			`✅ Evacuado ${newEvacuee.firstName} ${newEvacuee.lastName} ingresado en "${targetShelter?.name || "Refugio"}". Datos sincronizados con el Administrador.`,
		)
	}
	...
	const addNotice = (title: string, message: string, type: NoticeAlert["type"], shelterId?: string) => {
		const newNotice: NoticeAlert = {
			...
			author:
				...
				: currentRole === "admin" ? "Administración General"
				: "Comunicador Social",
			shelterId,
		}
