...
export const Header: React.FC = () => {
  const { currentUser, logout, currentRole, setCurrentRole, refugios, estadias, toastMessage } = useShelter();
              ...
              <div className="hidden xl:flex items-center gap-1 border-l border-zinc-200 dark:border-zinc-800 pl-2">
                <button
                  ...
                  className={`p-1 px-2 rounded text-[11px] font-bold ${
                    currentRole === 'administrador' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-zinc-800'
                  }`}
                ...
                <button
                  onClick={() => setCurrentRole('trabajador_social')}
                  className={`p-1 px-2 rounded text-[11px] font-bold ${
                    currentRole === 'trabajador_social' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-zinc-800'
                  }`}
                  title="Cambiar a Vista Trabajador Social"
                >
                  Trabajador Social
                </button>
              </div>
