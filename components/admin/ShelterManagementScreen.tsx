...
import { Building2, Plus, MapPin, Phone, UserCheck } from 'lucide-react';

export const ShelterManagementScreen: React.FC = () => {
  const { refugios, addRefugio, estadias } = useShelter();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (nombre.trim().length < 3 || nombre.trim().length > 120) {
      alert('El nombre del refugio debe tener entre 3 y 120 caracteres.');
      return;
    }

    if (!capacidad || Number(capacidad) <= 0 || Number(capacidad) > 10000) {
      alert('La capacidad debe ser entre 1 y 10.000.');
      return;
    }
            ...
            <Card key={r.id} className="border-t-4 border-t-blue-600 flex flex-col justify-between">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle className="text-base font-bold text-zinc-900 dark:text-zinc-100">{r.nombre}</CardTitle>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-50">
            <div className="flex items-center justify-between pb-4 border-b border-zinc-200 dark:border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-lg">Alta de Refugio (public.refugios)</h3>
              </div>
